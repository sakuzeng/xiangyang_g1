...
class VoiceInteraction(AudioRecorder):
    ...
    def run(self):
        try:
            # 1. 播报异常
            TTSClient.speak("财庙变财庙变/110kV.倚财线幺栋幺开关,WXH-813A光纤差动保护出口,跳闸（重合成功）(模拟)", wait=False)
...
class LocomotionController:
    ...
    def run_sequence(self):
        ...
        # 5. 前进 (用户输入距离)
        # if self._wait_for_confirmation("前进 (输入距离)"):
        try:
            dist_input = input("请输入前进距离(m): ").strip()
            dist = float(dist_input)
            self.move_distance(dist)
        except ValueError:
            print("⚠️ 输入无效，默认前进 0.6m")
    ...
    def execute_with_manual_data(self, manual_data):
        ...
        try:
            ...
            with robot_state.safe_arm_control(arm="left", source="integrated_demo", timeout=180.0):
                ...
                # 播报完成信息
                TTSClient.speak("报事故财庙变财庙变/110kV.倚财线幺栋幺开关,WXH-813A光纤差动保护出口,跳闸（重合成功）(模拟)", wait=False)
...
def main():
    ...
    print("\n" + "="*50)
    while True:
        choice = input("❓ 是否开始执行打电话动作序列? (y/n): ").strip().lower()
        if choice == 'y':
            break
        elif choice == 'n':
            print("❌ 用户取消动作执行")

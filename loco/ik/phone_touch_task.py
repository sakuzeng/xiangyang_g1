...
class PhoneTouchController:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            print(f"   ✅ Torso Z基准: {self.expected_torso_z:.3f}m")
            print(f"   ✅ 测量误差: {self.measurement_error}")
            print(f"   ✅ 手腕下倾角: {self.wrist_pitch:.3f} rad")
            if self.torso_x_range:
                print(f"   ✅ X范围限制: {self.torso_x_range}")
            if self.torso_y_range:
                print(f"   ✅ Y范围限制: {self.torso_y_range}")
...
def main():
    ...
    print("="*70)
    print("📱 手机触摸任务控制器 (升级版深度获取)")
    print("="*70)
    print(f"🔍 检测机器人状态: FSM ID={cur_id}, Mode={cur_mode}")
    
    # 根据 hanger_boot_sequence_run.py 的判断逻辑
    if cur_id == 801 and cur_mode is not None and cur_mode != 2:
        print("✅ 判定为: 走跑运控模式 (Run Mode)")
        # 走跑模式下的参数
    ...
    print(f"📐 测量误差修正: {MEASUREMENT_ERROR}")
    print(f"🤖 手腕下倾角: {WRIST_PITCH} rad")
    print(f"🛡️  X范围: {TORSO_X_RANGE}")
    print(f"🛡️  Y范围: {TORSO_Y_RANGE}")

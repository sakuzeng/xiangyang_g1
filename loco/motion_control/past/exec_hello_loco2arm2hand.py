...
class TTSClient:
    ...
    @staticmethod
    def speak(text, volume=50, wait=True, source="integrated"):
        """发送TTS请求并可选等待播放完成"""
        if not text:
            return
...
class G1PatrolDemo:
    ...
    def __init__(self, interface="eth0", turn_direction="right"):
        ...
        self.ANGULAR_VELOCITY = 0.80    # 🔧 降低角速度，提高精度 (0.25 → 0.20)
        # 🔧 开环补偿：由于实际总是转不到90度（只有70几度），人为增加目标角度到110度
        # self.TURN_ANGLE = math.radians(110)   # 90度 -> 110度
        self.TURN_ANGLE = math.pi / 2   # 90度
        
        # 控制精度
        self.POSITION_TOLERANCE = 0.05  # 位置容差(m)
        self.ANGLE_TOLERANCE = 0.08     # 🔧 适当放宽角度容差 (0.05 → 0.08)

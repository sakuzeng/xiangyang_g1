...
功能：
- 同时控制左右手臂和左右灵巧手
- 支持 4元组序列定义 (左臂, 右臂, 左手, 右手)
- 姿态文件导入参考 exec_show_sequence.py
"""
import sys
import time
import json
...
class FullBodyPoseSequence:
    ...
    def run_sequence(self, sequence: List[Tuple[str, str, str, str]], speed_factor: float = 1.0, pause_time: float = 1.0):
        """执行动作序列"""
        print("\n" + "="*70)
            ...
            with robot_state.safe_dual_arm_control(source="full_sequence", timeout=120.0):
                with robot_state.safe_hand_control(hand="left", source="full_sequence", timeout=120.0):
                    with robot_state.safe_hand_control(hand="right", source="full_sequence", timeout=120.0):
                        ...
                        for i, (l_arm, r_arm, l_hand, r_hand) in enumerate(sequence, 1):

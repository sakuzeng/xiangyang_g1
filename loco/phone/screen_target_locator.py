    ...
    def detect_screen_target(self, image: np.ndarray, target_index: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            if response.status_code == 200:
                ...
                raise YoloServiceError(f"YOLO服务错误: {response.status_code}")
        except requests.exceptions.Timeout:
            raise YoloServiceError("YOLO服务超时")
        except Exception as e:
            raise YoloServiceError(f"YOLO服务调用失败: {e}")
...
class ScreenTargetLocator:
    ...
    def __init__(self, 
                 ...
                 expected_torso_z: float = -0.17,
                 torso_z_tolerance: float = 0.05,
                 measurement_error: Optional[list] = None): # 🆕 测量误差用于显示
        """
        Args:
            yolo_server_url: YOLO服务地址
            expected_torso_z: 屏幕平面的Torso Z基准值 (米)
            torso_z_tolerance: Z值容差 (米)
            measurement_error: 测量误差 [x, y, z] (仅用于显示修正后坐标)
        """
        # 组件初始化
        ...
        self.expected_torso_z = expected_torso_z
        self.torso_z_tolerance = torso_z_tolerance
        self.measurement_error = np.array(measurement_error) if measurement_error else None
    ...
    def detect_and_locate(self, color_image: np.ndarray, depth_raw: np.ndarray, 
        ...
        Raises:
            YoloServiceError: YOLO服务异常
            TargetNotFoundError: 未找到目标
            DepthAcquisitionError: 深度获取失败
        """
        # 1. 调用YOLO服务
    ...
    def visualize_result(self, color_image: np.ndarray, result: Dict[str, Any]) -> np.ndarray:
        ...
        # 🆕 显示修正后坐标
        if self.measurement_error is not None:
            corrected = np.array(torso) + self.measurement_error
            label_corr = f"Corr: [{corrected[0]:.2f}, {corrected[1]:.2f}, {corrected[2]:.2f}]"
            cv2.putText(vis_image, label_corr, (actual_center[0]+12, actual_center[1]+15),
...
def main():
    ...
    parser.add_argument("--torso-z", type=float, default=None,
                       help="屏幕Torso Z基准值 (米), 默认根据mode自动选择")
    parser.add_argument("--z-tolerance", type=float, default=0.05,
                       help="Z值容差 (米)")
    parser.add_argument("--robot-mode", type=str, default="run",
                       choices=["run", "regular"],
                       help="机器人模式: run(走跑,默认) 或 regular(常规)")
    
    args = parser.parse_args()
    
    # 根据模式设置默认值
    expected_z = args.torso_z
    measurement_error = None
    
    if args.robot_mode == "run":
        if expected_z is None: expected_z = -0.17
        measurement_error = [0.01, -0.08, 0.25]
    else:
        if expected_z is None: expected_z = -0.15
        measurement_error = [-0.01, -0.07, 0.23]
    
    locator = ScreenTargetLocator(
        args.server,
        expected_torso_z=expected_z,
